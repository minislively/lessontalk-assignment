...
| 역할 | 이름 | 전화번호 | 확인할 수 있는 범위 | 작성 권한 |
|---|---|---|---|---|
| 회원 | 한지우 | `010-0000-0001` | 본인이 예약한 레슨과 피드백 | 작성 불가, 조회만 가능 |
| 프로 | 박프로 | `010-9000-0503` | 본인이 담당하는 레슨과 피드백 | 종료된 담당 레슨에 작성 가능 |
| 점주 | 점주 | `010-9000-0001` | `A-1001` 매장 전체 레슨과 피드백 | 소속 매장 레슨에 작성 가능 |

#### 검토자 확인 순서

1. **회원 계정**으로 로그인합니다. 본인 레슨만 보이고, 피드백 작성 화면이 없어야 합니다.
2. **프로 계정**으로 로그아웃 후 로그인합니다. 담당 레슨을 확인하고, 종료된 레슨 상세에서 피드백을 작성할 수 있어야 합니다.
3. **점주 계정**으로 로그인합니다. `A-1001` 매장의 전체 레슨을 확인하고, 매장 레슨에 대한 피드백 작성 및 동기화 기능을 확인합니다.
4. 회원 계정으로 다시 로그인해 프로가 작성한 피드백을 조회합니다.

외부 시스템의 예약은 5분 주기로 변동될 수 있으므로 레슨 개수와 상태는 실행 시점마다 달라질 수 있습니다. 중요한 확인 기준은 개수가 아니라 **역할별 조회 범위와 작성 가능 여부**입니다.

회원가입 화면에서 새 계정을 만들면 기본적으로 매장 소속이 없는 회원으로 생성됩니다. 임의의 `storeId`를 회원가입 요청에 넣어도 매장에 자동 가입되지 않으며, 실제 매장 접근 권한은 조직에서 별도로 부여해야 합니다.

기본 포트가 이미 사용 중이면 API 호스트 포트를 바꿔 실행할 수 있습니다.
